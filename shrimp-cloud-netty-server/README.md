...
| 配置名             | 用途    | 是否必填 | 默认值      | 备注                   |
|-----------------|-------|------|----------|----------------------|
| netty.nettyPort | 服务端口  | 否    | 10000    | 端口号                  |
| netty.isWriteLog      | 是否写日志 | 否    | true     | 是否写日志                |
| netty.appName         | 服务名称  | 否    | nettyApp | 服务名称，会在数据对象中体现，可入库备查 |
| netty.serverType      | 服务类型  | 否    | tcp      | 服务类型，tcp或者udp        |
...
注解说明：
- `@NettyHandlerMethod`注解支持输入输出类型转换，即字符串和字节数组等类型互转，具备灵活性。
